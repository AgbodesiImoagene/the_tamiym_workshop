...
**Status:** Not started  
**Risk:** High  
**Blocked by:** TTW-003, TTW-010, TTW-011, TTW-013, TTW-015, TTW-021, TTW-023, TTW-043  
...
5. Define alert routing and inhibition/deduplication by severity and environment; provision a safe local/staging receiver and document the production secret/receiver interface.
...
7. Run controlled staging failure drills, including exporter loss, delayed queue work, invalid webhook, failed media scan and failed payout/refund simulation; tune thresholds from evidence and record detection/recovery times.
...
- [ ] Versioned dashboards and alert rules provision and validate automatically in local/staging environments.
- [ ] Every paging alert links to a reviewed runbook and reaches the expected staging receiver during a controlled drill.
