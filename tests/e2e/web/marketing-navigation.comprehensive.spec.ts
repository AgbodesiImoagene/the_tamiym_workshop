...
describeViewportMatrix('Web marketing navigation @comprehensive @web', () => {
  test('home page hero, CTAs, and header controls', async ({ page }) => {
    ...
    const viewWorkshop = page.getByRole('link', { name: /View Workshop/i });
    if (await viewWorkshop.isVisible()) {
  ...
  test('header and footer links navigate internally', async ({ page }) => {
    await page.goto('/');
    await openMobileNavIfPresent(page);
    await exerciseInternalLinks(page, page.locator('header'), { maxLinks: 4 });
    await exerciseInternalLinks(page, page.locator('footer'), { maxLinks: 4 });
  });
